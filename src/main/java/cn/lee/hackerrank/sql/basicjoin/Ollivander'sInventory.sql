...
select w.id,wp.age, w.coins_needed ,w.power from
  Wands w join Wands_Property wp on w.code = wp.code and wp.is_evil=0
  join (
         select wp.age,w.power,min(w.coins_needed) coins_needed from Wands w join Wands_Property wp on w.code = wp.code and wp.is_evil=0
         group by wp.age,w.power) t on w.coins_needed = t.coins_needed and wp.age=t.age and w.power=t.power
